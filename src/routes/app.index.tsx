...
function buildRecommendation(
  ...
  const unstudied = documents.find(
    (d) => !overview.perMaterial.some((m) => m.documentId === d.id),
  );
  if (unstudied) {
    return {
    ...
    <div className="mx-auto max-w-[1200px] space-y-8">
          ...
          {todaySession ? (
            <>
              <p className="mt-5 font-mono text-xs text-muted-foreground">
                {todaySession.completedAt ? formatDate(todaySession.completedAt) : t("overview.inProgress")}
              </p>
            ...
            <>
              <p className="mt-5 font-mono text-xs text-muted-foreground">{t("overview.noSessions")}</p>
              <h2 className="mt-2 break-words text-2xl tracking-tight md:text-3xl">
                {documents[0]?.title}
              </h2>
              <p className="mt-4 text-sm text-muted-foreground">
                {t("overview.startFirst")}
              </p>
              <Link
                to="/app/questions/$documentId"
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                {flashcardOverview.nextDueAt
                  ? t("overview.flashcardsNextReview", {
                      date: formatReviewDate(flashcardOverview.nextDueAt, locale),
                    })
      ...
      <section aria-labelledby="recent-heading" className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <Link to="/app/materials" className="font-mono text-xs text-muted-foreground hover:text-lime">
            {t("common.viewAll")} →
          </Link>
        </div>
        ...
        {documents === null ? (
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {documents.slice(0, 3).map((doc) => {
              ...
              return (
                <AppCard key={doc.id}>
                  <AppLabel>{formatDate(doc.createdAt)}</AppLabel>
